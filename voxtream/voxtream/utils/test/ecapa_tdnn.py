...
class ECAPA_TDNN(nn.Module):
    def __init__(
        ...
        if feat_type == "fbank":
            ...
            # s3prl probes optional upstreams on import and emits warnings for missing extras.
            # We only need wavlm here, so silence the optional espnet_hubert warning logger.
            logging.getLogger("s3prl.upstream.espnet_hubert.expert").setLevel(
                logging.ERROR
            )
            self.feature_extract = torch.hub.load(
                "s3prl/s3prl", feat_type, trust_repo=True, verbose=False
            )
...
def init_ecapa(
    ...
    """Initialize the ECAPA-TDNN model with a pre-trained checkpoint."""
    model = ECAPA_TDNN_SMALL(feat_dim=feat_dim, feat_type=feat_type)
