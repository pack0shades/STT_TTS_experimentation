...
class BasePhonemizer(abc.ABC):
    ...
    @abc.abstractmethod
    def _phonemize(self, text, separator, language):
    ...
    def phonemize(
        self, text: str, separator="|", language: Optional[str] = None
    ) -> str:  # pylint: disable=unused-argument
        """Returns the `text` phonemized
        ...
        Returns:
            (str): Phonemized text
        """
        texts, punctuations = self._phonemize_preprocess(text)
        phonemized: List[str] = []
        for text_chunk in texts:
            p = self._phonemize(text_chunk, separator, language)
            phonemized.append(p)
